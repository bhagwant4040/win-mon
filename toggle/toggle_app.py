"""
winMon Privacy Toggle — lets the employee turn this PC's camera and
microphone monitoring access off/on with one click.

Purely local: flips a value under HKEY_CURRENT_USER that winMon.exe checks
right before it would open the webcam or mic (see _cam_mic_locally_blocked()
in agent/agent.py). No server round-trip, no admin action needed, doesn't
require winMon to be running to flip the switch — it just takes effect the
next time winMon itself checks (within ~20s for camera, at the next Talk
session attempt for mic). Does NOT affect screen monitoring/Live View, and
does NOT block the admin's ability to talk TO this PC over the speaker —
only this PC's camera and this PC's outgoing microphone audio.
"""
...
class App:
    def __init__(self, root):
        ...
        tk.Label(root, text='Camera & Microphone', font=('Segoe UI', 15, 'bold'),
        ...
        self.btn = tk.Button(root, text='', font=('Segoe UI', 12, 'bold'),
                              ...
                              command=self.toggle)
        self.btn.pack()
        tk.Label(root, text='Only affects this PC. Screen monitoring is unaffected.',
